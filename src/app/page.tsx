...
export default async function Home() {
  ...
  const portfolioItems: PortfolioItem[] = [
    ...
    ...projects.map((p: any): PortfolioItem => ({
      ...
      path: p.type === "PROJECTS" ? `/projects/${p.slug}` : `/labs/${p.slug}`,
      type: p.type || "PROJECTS",
      status: p.status,
      ...
      iconColor: p.type === "PROJECTS" ? "text-primary" : "text-purple-400",
      iconBg: "bg-surface",
      hoverColor: p.type === "PROJECTS" ? "group-hover:text-primary-light" : "group-hover:text-purple-300",
      hoverBorder: p.type === "PROJECTS" ? "group-hover:border-primary/50" : "group-hover:border-purple-400/50",
      badgeColor: p.type === "PROJECTS" ? "bg-blue-500/10 text-blue-400 border-blue-500/20" : "bg-purple-500/10 text-purple-400 border-purple-500/20",
      statusColor: p.status === "FINISHED" ? "bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.4)]" : "bg-yellow-400 shadow-[0_0_8px_rgba(250,204,21,0.4)]",
    })),
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
